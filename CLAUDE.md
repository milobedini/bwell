...
- **Cue-based child animations:** For components that play an animation when the parent says "now" (e.g. a celebratory burst), expose an imperative handle via `useImperativeHandle` and let the parent call `ref.current?.action()`. Don't pass a counter prop and watch it in an effect. See `BloomBurst` for reference.
- **Icons over unicode glyphs:** Never use unicode glyphs (▲ ◆ ✕ ✓ ⟳ ↑ ↓ →) as UI iconography — rendering is inconsistent across iOS/Android/web font stacks and there is no control over stroke weight, baseline, or size. Use `@react-native-vector-icons/material-design-icons` (MDI) via the existing `Icon` import, or an existing SVG component. Reserve text for figures (counts, percentages) and for labels. When an icon flips with state (e.g. rollup `check-circle-outline` when fresh vs `sync-alert` when stale), derive the name from a small `iconFor` function; when the icon represents the concept regardless of state, keep it stable. See `AttentionBanner` and `OutcomesSparkline` for reference.
