...
import { DEFAULT_STALLED_FILTERS, useAdminStalledAttempts } from '@/hooks/useAdminStalledAttempts';
...
const StalledAttemptsScreen = () => {
  const {
    ...
    hasNextPage,
    isFetchingNextPage
  } = useAdminStalledAttempts(DEFAULT_STALLED_FILTERS);

  const items: AdminStalledAttemptRowType[] = useMemo(() => data?.pages.flatMap((p) => p.items) ?? [], [data]);

  const renderItem = useCallback<ListRenderItem<AdminStalledAttemptRowType>>(
    ({ item, index }) => {
      const isLast = index === items.length - 1;
      return (
        <View className="mx-4 overflow-hidden" style={pickRowStyle(index === 0, isLast)}>
          <StalledAttemptRow row={item} isLast={isLast} />
        </View>
      );
    },
    [items.length]
  ...
  return (
    <View className="flex-1 bg-sway-dark" testID="admin-stalled-attempts-screen">
      <ContentContainer>
        <View className="py-3">
          ...
          >
            Started, no activity for 7+ days. Oldest first.
          </ThemedText>
        </View>
      </ContentContainer>
...
export default StalledAttemptsScreen;
